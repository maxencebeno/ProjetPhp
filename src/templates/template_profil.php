			...
			if (in_array($extension_upload, $extensions_autorisees)){
				if($_FILES['file']['size']<=5242880){
					...
					if($deja_photo_perso==false){ // Si premier upload on crée une entrée dans la table "images"
						...
						$req->closeCursor();
					}else{ // Sinon; l'entrée existe déjà, on la met juste à jour.
						$req = $bdd->prepare('UPDATE images SET chemin_image = ? WHERE id_user = ?');
						$req->execute(array($emplacement, $_SESSION['id_user']));
						$req->closeCursor();
					}
				...
				$erreur_upload = '<p style="color: red">La taille de l\'image est trop grande. Elle doit être inférieure à 5 Mo.</p>';
				}
			}
			else{
				$erreur_upload = '<p style="color: red">Format de fichier invalide. Réessayer avec un type autorisé.</p>';
			}
		}
		else{
			$erreur_upload = '<p style="color: red">Erreur fichier. Réessayez avec une autre image.</p>';
		}
	}
